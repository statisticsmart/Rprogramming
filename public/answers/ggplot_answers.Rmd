...
```{r, fig.cap="Figure 1", comment=NA, message=FALSE, comment=FALSE}
ggplot(gapminder, aes(y=lifeExp, x=gdpPercap, col=continent))+geom_point()

```

It's impossible to see whats going on in Figure 1! Now lets look at the same graph with both axes scaled by factors of 10 (in other words, on logarithmic axes).


```{r, comment=NA, message=FALSE, comment=FALSE, fig.cap="Figure 2"}
ggplot(gapminder, aes(y=log(lifeExp), x=log(gdpPercap), col=continent))+geom_point()
...
## Extra: Create interactive graphics
...
You should include Figure caption  for all graphs. It is important to interpret all graphs.